...
// 获取图书长评列表
server.tool(TOOL.LIST_BOOK_REVIEWS, "list book reviews", {
...
// 搜索电影或电视剧
server.tool(TOOL.SEARCH_MOVIE, 'search movies or tvs from douban by query', {
...
// 获取电影长评列表
server.tool(TOOL.LIST_MOVIE_REVIEWS, "list movie reviews", {
...
// 获取电视剧长评列表
server.tool('list-tv-reviews', "list tv reviews", {
...
// 浏览图书详情
server.tool(TOOL.BROWSE, "open default browser and browse douban book detail", {
    id: z.string().describe('douban book id, e.g. "1234567890"')
}, async (args) => {
    if (!args.id) {
        throw new McpError(ErrorCode.InvalidParams, "douban book id must be provided");
...
// 获取小组话题列表
server.tool(TOOL.LIST_GROUP_TOPICS, "list group topics", {
...
// 获取小组话题详情
server.tool(TOOL.GET_GROUP_TOPIC_DETAIL, "get group topic detail", {
