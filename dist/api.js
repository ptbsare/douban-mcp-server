...
export async function getBookReviews(params) {
    const res = await requestFrodoApi(`/book/${params.id}/reviews`);
    return res;
}
// 搜索电影或电视剧API
...
export async function getGroupTopics(params) {
    ...
    let topics = (res.topics || []).filter(_ => !_.is_ad);
    if (params.tags) {
...
export async function getGroupTopicDetail(params) {
    const res = await requestFrodoApi(`/group/topic/${params.id}`);
    return res;
}
const FAKE_HEADERS = {
...
const requestFrodoApi = async (url) => {
...
const getFrodoSign = (url, date, method = 'GET') => {
